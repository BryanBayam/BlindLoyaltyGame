 ...
 *
 * SCENE4_TEXT_LEAD_SECONDS : Number of seconds the text should finish
 *                            before the voice clip ends.
 * SCENE4_MIN_TEXT_DURATION : Minimum allowed total duration for text reveal.
 */
#define SCENE4_TEXT_LEAD_SECONDS 2.0f
#define SCENE4_MIN_TEXT_DURATION 0.15f

/*
 * Runtime states for Scene 4.
 *
 * SCENE4_FADE_IN  : Fade from black into the scene
 * SCENE4_TEXT     : Display narrator dialogue with typewriter effect
 * SCENE4_FADE_OUT : Fade back to black after dialogue completes
 * SCENE4_DONE     : Scene has finished
 */
typedef enum {
    SCENE4_FADE_IN,
    SCENE4_TEXT,
...
/*
 * Initialize Scene 4 with default visuals, text, audio,
...
static inline void InitScene4(Scene4* scene) {
    ...
    scene->narratorDuration =
        Scene4LoadVoiceWithDuration("audio/Voice/Scene 4/Narrator.mp3", &scene->narratorVoice);

    scene->narratorPlayed = false;
    scene->textSpeed =
        Scene4CalcTextSpeed(scene->dialogue, scene->narratorDuration, scene->textSpeed);
}

/*
 * Update Scene 4 each frame.
 *
 * Scene flow:
...
/*
 * Draw Scene 4 based on the current state.
 *
 * Visual behavior:
...
/*
 * Release all resources owned by Scene 4.
